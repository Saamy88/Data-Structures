...
namespace CS3358_Fall2014_Assign08
{
   ...
   // Pre:  (none)
   // Post: Adds new value to the heap. The value will do a reheapification
   //       upward until he finds a parent with a higher priority.
   {
	   if( size() >= capacity)
		   resize( p_queue::size_type(2 * capacity) + 1);
	   ...
	   if(used > 1)
	   {
		   ...
		   do
		   {
			   ...
			   if(big_child_priority(parent_index(temp_used - 1)) >
			                                     parent_priority(temp_used - 1))
			   {
   ...
   void p_queue::pop()
   // Pre:  (none)
   // Post: Deletes the value with highest priority. Then, sets a new value
   //   with the highest priority after the one deleted.
   {
	  assert( size() > 0);
   ...
   // Pre:  (none)
   // Post: Returns the number of value in the heap.
   ...
   {
   // Pre:  (none)
   // Post: Returns true if there are no values in the heap, otherwise returns
   //       false.
      if(used < 1) 
		  return true;
	  else 
   ...
   // Pre:  (none)
   // Post: Returns the data of the value with highest priority
   {
	  assert(used > 0);
