...
void MakeDistinctMirrorPairs(Node*& headPtr)
{
	...
	while(headPtr != 0)
	{
		Node* cur = headPtr->link;
		Node* pre = headPtr;
		while(cur != 0)
		{
			while(cur != 0 && cur->data != headPtr->data)
			{
				pre = cur;
				cur = cur->link;
			}
	...
	while(headPtr != 0)
	{
		...
		while(cur != 0)
		{
			...
			if(cur != 0 && cur->data == headPtr->data)
			{
				pre->link = cur->link;
				cur->link = mirror;
				mirror = cur;
			...
			if(cur == 0)
			{
				...
				newNodePtr->link = mirror;
				mirror = newNodePtr;
				break;
			}
		}
		if(cur == 0 && headPtr->link == 0 )
		{
			Node* newNodePtr = new Node;
			newNodePtr->data = headPtr->data;
			newNodePtr->link = mirror;
