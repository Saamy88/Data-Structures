...
// definition of MakeAllOddAndAllEven of Assignment 5 Part 2
void MakeAllOddAndAllEven(Node*& head1,Node*& head2)
{

	Node* tempE = NULL;
	Node* tempO = NULL;

	// start at beginning of lists to find all evens in odd list
	Node* current = head1;
	Node* cur1pre = head1;
	Node* current2 = head2;
	Node* cur2pre = head2;

	while (current !=0)// as long as there are nodes to traverse
	{
		while (current != 0 && current->data % 2 != 0)
		{
			// go to the first even number in the odd list
			cur1pre = current;
			current = current->link;
		}
		while(current2 != 0 && current2->data % 2 == 0)
		{
			// go to the first odd number in the even list
			cur2pre = current2;
			current2 = current2->link;
		}
		while(current != 0 && current->data % 2 ==0)
		{
			//place the even node after the last even and before the first odd
			tempE = current;
			cur1pre = current->link;
			current = current->link;
			cur2pre = tempE;
			tempE->link = current2;
		}
	...
	// start over at both lists to find all odds in the even list
	current = head1;
	cur1pre = head1;
	current2 = head2;
	cur2pre = head2;
	while(current2 !=0)// as long as there are nodes to traverse
	{
		while (current2 != 0 && current2->data % 2 == 0)
		{
			// go to the first odd number in the even list
		...
		while(current != 0)
		{
			//go to the last spot in the only odd list now
			cur2pre = current2;
			current2 = current2->link;
		}
		while(current != 0 && current->data % 2 ==0)
		{
			//place the odd node after the last odd and before the first even
			tempE = current;
			cur1pre = current->link;
			current = current->link;
			cur2pre = tempE;
