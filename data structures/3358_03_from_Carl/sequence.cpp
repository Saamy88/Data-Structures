...
void sequence::resize(size_type new_capacity)
{
	...
	double * newDataArray = new double [capacity];
...
{
	if(used >= capacity)
		resize(size_type(capacity * 1.25) + 1);
...
sequence& sequence::operator=(const sequence& source)
{
	if(this != &source)
	{
		value_type* newDataArray = new value_type[source.capacity];

		for(size_type i = 0; i < source.used; ++i)
		{
			newDataArray[i] = source.data[i];
		}
